...
async function startServer() {
  ...
  app.post("/api/transcript", async (req, res) => {
    ...
    try {
      ...
      // Heuristic to detect auto-generated transcripts (usually no punctuation and all lowercase)
      const sampleText = transcript.slice(0, 10).map(t => t.text).join(' ');
      const isLikelyAutoGenerated = !/[A-Z]/.test(sampleText) && !/[.!?]/.test(sampleText);

      // Also try to get metadata for better indexing
      ...
      try {
        ...
        const video = searchResults.items.find(item => item.type === 'video') as any;
        if (video) {
        ...
        // Fallback to oembed for title if ytsr failed
      ...
      if (isLikelyAutoGenerated) {
        return res.json({ 
          ...
          uploadedAt,
          fallback: true, 
          message: "Auto-generated transcript detected. Using AI-generated alternative for better quality and punctuation..." 
        });
      }

      res.json({ transcript, title, description, uploadedAt });
    } catch (error: any) {
      ...
      try {
        ...
        console.warn("Fallback metadata fetch failed", e);
      }
